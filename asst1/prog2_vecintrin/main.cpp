...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    __cs149_vec_float vec_x, vec_z, vec_max;
    __cs149_vec_int vec_y, vec_zero, vec_ones;
    __cs149_mask mask_all = _cs149_init_ones();
    __cs149_mask mask_z = _cs149_init_ones(0);

    // initialize the vectors
    _cs149_vload_float(vec_x, values+i, mask_all);
    _cs149_vload_int(vec_y, exponents+i, mask_all);
    _cs149_vset_int(vec_zero, 0, mask_all);
    _cs149_vset_int(vec_ones, 1, mask_all);
    _cs149_vset_float(vec_max, 9.999999f, mask_all);

    // set up the result vector (vec_z)
    _cs149_veq_int(mask_z, vec_y, vec_zero, mask_all);
    // set z to 1 if y == 0
    _cs149_vset_float(vec_z, 1.0, mask_z);
    mask_z = _cs149_mask_not(mask_z);
    // else set z equal to x
    _cs149_vmove_float(vec_z, vec_x, mask_z);
    
    _cs149_vsub_int(vec_y, vec_y, vec_ones, mask_z);
    while(_cs149_cntbits(mask_z) != 0) {
      // if y > 0; z = z * x
      _cs149_vgt_int(mask_z, vec_y, vec_zero, mask_z);
      _cs149_vmult_float(vec_z, vec_z, vec_x, mask_z);
      // y = y - 1
      _cs149_vsub_int(vec_y, vec_y, vec_ones, mask_z);
    }
    // if z > 9.999999f; z = 9.999999
    _cs149_vgt_float(mask_z, vec_z, vec_max, mask_all);
    _cs149_vset_float(vec_z, 9.999999f, mask_z);

    // store the result
    _cs149_vstore_float(output+i, vec_z, mask_all);
  }
}
...
float arraySumVector(float* values, int N) {
  ...
  float sum = 0;
  __cs149_vec_float sum_array, array;
  __cs149_mask maskall = _cs149_init_ones();
  _cs149_vset_float(sum_array, 0, maskall);

  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    _cs149_vload_float(array, values+i, maskall);
    _cs149_vadd_float(sum_array, sum_array, array, maskall);
  }

  for(int i=0; i<VECTOR_WIDTH; i++) {
    sum += sum_array.value[i];
  }
