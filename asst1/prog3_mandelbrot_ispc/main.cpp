...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        mandelbrot_ispc_t(x0, y0, x1, y1, width, height, maxIterations, output_ispc);
        double endTime = CycleTimer::currentSeconds();
        minISPC_t = std::min(minISPC_t, endTime - startTime);
    }

    printf("[mandelbrot ispc1]:\t\t[%.3f] ms\n", minISPC * 1000);
    printf("[mandelbrot ispc2]:\t\t[%.3f] ms\n", minISPC_t * 1000);
