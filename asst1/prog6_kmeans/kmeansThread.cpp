...
typedef struct {
  ...
  int mstart, mend; // Used to control which data points are processed
} WorkerArgs;
 ...
 */
void computeAssignmentsTask(WorkerArgs *const args) {
  double *minDist = new double[args->M/16];
  int start = args->mstart;
  // Initialize arrays
  for (int m = start; m < args->mend; m++) {
    minDist[m-start] = 1e30;
...
void computeAssignmentsThread(WorkerArgs *const args) {
  // Create a thread for each cluster centroid
  std::thread threads[16];
  WorkerArgs *argsCopy = new WorkerArgs[16];

  for (int i = 0; i < 16; i++) {
    argsCopy[i] = *args; // Copy the original args
    argsCopy[i].mstart = i * (args->M / 16);
    argsCopy[i].mend = min(args->M, (i + 1) * (args->M / 16));
    threads[i] = std::thread(computeAssignmentsTask, &argsCopy[i]);
  }
  // Join all threads
  for (int i = 0; i < 16; i++) {
    threads[i].join();
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  printf("Total times:\n\t\tAssignment: %.3f ms\n\t\tCentroid: %.3f ms\n\t\tCost: %.3f ms\n",
         totalAssignmentTime * 1000, totalCentroidTime * 1000,
         totalCostTime * 1000);
